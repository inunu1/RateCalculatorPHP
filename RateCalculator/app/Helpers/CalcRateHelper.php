...
class CalcRateHelper
{
    ...
    COALESCE(ranked_rates.latest_rate, players.regist_rating) AS latest_rate,
    CASE 
        WHEN ranked_rates.player_id IS NOT NULL THEN TRUE 
        ELSE FALSE 
    END AS has_game_experience
FROM
    players
LEFT JOIN (
    SELECT
...
ON players.id = ranked_rates.player_id AND ranked_rates.rn = 1
ORDER BY players.id;";
    }
}
